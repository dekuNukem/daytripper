...
uint8_t address = AddressDefault;
...
DistanceMode distance_mode = Unknown;
...
DistanceMode getDistanceMode(void)
...
uint8_t checkTimeoutExpired(void)
...
uint8_t VL53L1X_init()
{
  ...
  setMeasurementTimingBudget(33333);
...
void writeReg(uint16_t reg, uint8_t value)
{
  ...
  last_status = HAL_I2C_Mem_Write(&hi2c1, address, reg, 2, &value, 1, 17);
}
...
void writeReg16Bit(uint16_t reg, uint16_t value)
{
  ...
  buf[0] = (value >> 8) & 0xFF;
  buf[1] = value & 0xFF;
  last_status = HAL_I2C_Mem_Write(&hi2c1, address, reg, 2, buf, 2, 17);
}
...
void writeReg32Bit(uint16_t reg, uint32_t value)
{
  ...
  buf[2] = (value >> 8) & 0xFF;
  buf[3] = value & 0xFF;
  last_status = HAL_I2C_Mem_Write(&hi2c1, address, reg, 2, buf, 4, 17);
}
...
uint8_t setDistanceMode(DistanceMode mode)
{
...
void readResults()
{
  ...
  results.stream_count = result_buf[2];

  results.dss_actual_effective_spads_sd0  = (uint16_t)result_buf[3] << 8; // high byte
  results.dss_actual_effective_spads_sd0 |=           result_buf[4];      // low byte

  results.ambient_count_rate_mcps_sd0  = (uint16_t)result_buf[7] << 8; // high byte
  results.ambient_count_rate_mcps_sd0 |=           result_buf[8];      // low byte

  results.final_crosstalk_corrected_range_mm_sd0  = (uint16_t)result_buf[13] << 8; // high byte
  results.final_crosstalk_corrected_range_mm_sd0 |=           result_buf[14];      // low byte

  results.peak_signal_count_rate_crosstalk_corrected_mcps_sd0  = (uint16_t)result_buf[15] << 8; // high byte
  results.peak_signal_count_rate_crosstalk_corrected_mcps_sd0 |=           result_buf[16];      // low byte
}
